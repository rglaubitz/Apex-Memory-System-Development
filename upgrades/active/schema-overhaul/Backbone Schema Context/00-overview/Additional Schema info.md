...
## ⚠️ IMPORTANT: THIS DEFINES STRUCTURE, NOT IMPLEMENTATION

**This document defines:**
- ✅ Entity types (Tractor, Driver, FuelTransaction)
- ✅ Properties (unit_number, make, model, service_date)
- ✅ Relationships (Driver ASSIGNED_TO Tractor)
- ✅ Database distribution (what data goes where)

**This document does NOT define:**
- ❌ How to extract data from PDFs (implementation phase)
- ❌ How to calculate metrics (agent logic)
- ❌ How to detect patterns (application features)

**See [SCHEMA-PHILOSOPHY.md](SCHEMA-PHILOSOPHY.md) for complete design principles.**
...
**Key Concept:** These aren't 5 separate databases - they're 5 memory types forming ONE unified intelligence:

- **Neo4j** = Relationship Memory (who knows whom, what connects to what)
- **PostgreSQL** = Factual Memory (what happened, when, how much)
- **Qdrant** = Semantic Memory (document understanding, concept search)
- **Redis** = Working Memory (current state, real-time awareness)
- **Graphiti** = Temporal Memory (how things change, pattern recognition)

**The schema ensures these 5 memory types work together seamlessly.**
...
## 8. CROSS-DATABASE IDENTITY

**Critical Principle:** Same entity must be recognizable across ALL 5 databases.

### Example: Unit #6520 (Tractor) Across Databases
...
GET truck:6520:location        # Current GPS coordinates
GET truck:6520:status          # Current status (active/maintenance)
...
**Key Insight:** The `unit_number` field is consistent across all 5 databases, enabling:
- LLMs/agents to query across databases seamlessly
- Multi-database aggregations (fuel cost from PostgreSQL + location from Redis)
- Unified entity understanding (same truck recognized everywhere)

### Primary Key Strategy

```yaml
Tractor:          unit_number (string, 4 digits, "6520")
Trailer:          unit_number (string, "T-001")
...
**Schema's Role:** Define STRUCTURE for temporal data (properties that track time)

**Graphiti's Role (implementation):** Execute QUERIES on temporal data

### Bi-Temporal Tracking Structure

All entities that change over time include these properties:
...
```cypher
// Schema defines: financing_status property + valid_from/valid_to fields

// Truck 6520 was financed (historical state)
(Tractor:Unit6520 {
...
// Truck 6520 is now owned (current state)
(Tractor:Unit6520 {
...
### Temporal Queries (Graphiti Implementation - NOT Schema)

```python
# IMPLEMENTATION (NOT schema design)
# Graphiti executes temporal queries like:
...
**What Schema Defines:**
- ✅ Which properties are temporal (status, driver assignment, etc.)
- ✅ Structure of valid_from/valid_to fields
- ✅ Which entities get stored in Graphiti

**What Schema Does NOT Define:**
- ❌ How to query temporal data (Graphiti SDK calls)
- ❌ How to detect patterns (application logic)
- ❌ How to visualize changes over time (UI/UX)

---

## 10. IMPLEMENTATION PRIORITIES

**Note:** These are implementation phases, NOT schema design. Schema defines WHAT exists; implementation defines HOW to build it.
...
✅ Cross-database identity strategy defined
